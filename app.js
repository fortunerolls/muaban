...
async function sendWithRetry(callBuilder, kind="med"){
  // Kiểm tra VIC cho phí gas trước (ước tính thô)
  try{
    const bal = await providerWrite.getBalance(account);
    // thử với gasPrice cao nhất kế hoạch để tránh thiếu VIC
    const maxGwei = LEGACY_GAS_STEPS[LEGACY_GAS_STEPS.length-1];
    const limit = (kind==="light")?GAS_LIMIT_LIGHT: (kind==="heavy")?GAS_LIMIT_HEAVY: GAS_LIMIT_MED;
    const need = limit.mul(ethers.utils.parseUnits(String(maxGwei),"gwei"));
    if (bal.lt(need)){
      // vẫn cho thử gửi, nhưng cảnh báo trước
      alert(`Ví VIC còn ít (≈ ${ethers.utils.formatEther(bal)} VIC). Có thể thiếu phí gas khi gửi giao dịch.`);
    }
  }catch(_){}

  // 1) Thử legacy nhiều mức gasPrice (type 0)
  for(const g of LEGACY_GAS_STEPS){
    try{
      const ov = buildLegacy(kind,g);
      const tx = await callBuilder(ov);
      return await tx.wait();
    }catch(e){
      const msg = String(e?.message||"");
      // nếu không phải do phí/gas/tx type thì dừng ngay
      if(!(e?.code===-32603 || /underpriced|fee cap too low|replacement|intrinsic gas too low|nonce/i.test(msg))){
        throw e;
      }
      // tiếp tục bump
    }
  ...
  // 2) Fallback EIP-1559
  try{
    const ov = await buildEIP1559(kind);
    const tx = await callBuilder(ov);
    return await tx.wait();
  }catch(e1559){
    // 3) Phương án cuối: để MetaMask tự estimate (không override gì)
    try{
      const tx2 = await callBuilder({});
