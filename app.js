...
(() => {
  ...
  async function refreshWalletUI(){
    ...
    }catch(err){ toast(`Không đọc được số dư: ${prettifyError(err)}`,"error",6500); }
  }
  ...
  async function ensureRegistered(){
    try{
      ...
      if(cur.lt(fee)){
        const tx0 = await vin.approve(MUABAN_ADDR, fee);
        toast(`Approve phí đăng ký ~${ethers.utils.formatUnits(fee, vinDecimals)} VIN… <a href="${explorerTx(tx0.hash)}" target="_blank" rel="noopener">tx</a>`,"info",6000);
        await tx0.wait();
      }
      try{ await muaban.callStatic.payRegistration({ from:account }); }catch(simErr){
        const r = mapReasonToVN(extractReason(simErr)); if(r) throw new Error(r);
        throw simErr;
      }
      const tx = await muaban.payRegistration({ from:account, gasLimit: ethers.BigNumber.from("200000") });
      toast(`Đăng ký… <a href="${explorerTx(tx.hash)}" target="_blank" rel="noopener">tx</a>`,"info",6000);
      await tx.wait();
      toast("Đăng ký ví thành công.","success");
      return true;
    }catch(err){
      toast(`Đăng ký thất bại: ${prettifyError(err)}`,"error",8500);
      return false;
    }
  }
  ...
  async function submitCreate(){
    try{
      ...
      let tx;
      try{ await muaban.callStatic.createProduct(name, descriptionCID, imageCID, priceVND, deliveryDaysMax, payoutWallet, active, overrides); }
      catch(simPre){ const r = mapReasonToVN(extractReason(simPre)); if(r) throw new Error(r); }
      try{
  ...
  async function submitBuy(){
    try{
      ...
      let tx2;
      try{ await muaban.callStatic.placeOrder(BUYING_PID, ethers.BigNumber.from(qty.toString()), vinPerVNDWei, buyerInfoCipher, { from:account, gasLimit: ethers.BigNumber.from("900000") }); }
      catch(simPre){ const r = mapReasonToVN(extractReason(simPre)); if(r) throw new Error(r); }
      try{ tx2 = await muaban.placeOrder(BUYING_PID, ethers.BigNumber.from(qty.toString()), vinPerVNDWei, buyerInfoCipher, { from:account, gasLimit: ethers.BigNumber.from("900000") }); }catch(sendErr){ try{ await muaban.callStatic.placeOrder(BUYING_PID, ethers.BigNumber.from(qty.toString()), vinPerVNDWei, buyerInfoCipher, { from:account }); }catch(simErr){ const r=mapReasonToVN(extractReason(simErr)); if(r) throw new Error(r); } throw sendErr; }
